...
def eval_edge_prediction(model, negative_edge_sampler, data, n_neighbors, batch_size=200,
  ...
  with torch.no_grad():
    ...
    for k in iterator:
      ...
      vap = average_precision_score(true_label, pred_score)
      auc = roc_auc_score(true_label, pred_score)

      val_ap.append(vap)
      val_auc.append(auc)
      if not progress:
        print("batch", k, "of ", num_test_batch, "val_ap", vap, "val_auc", auc)
  
  if not progress:
    print("mean ap", np.mean(val_ap), "mean auc", np.mean(val_auc))
